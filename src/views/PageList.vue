...
const submitFilter = async (): Promise<void> => {
  ...
  interviews.value = await getAllInterview(true)
...
const getAllInterview = async <T extends IInterview>(isFilter?: boolean): Promise<T[]> => {
  let getData
...
<template>
  <div class="main">
    ...
    <app-message v-else-if="!isLoading && !interviews.length" severity="info" class="message"
      >На данный момент список пуст</app-message
    >
    <app-datatable v-else :value="interviews">
      ...
      <app-column field="vacancyLink" header="Вакансия">
        <template #body="slotProps">
          <a :href="slotProps.data.vacancyLink" target="_blank">Страница вакансии</a>
        </template>
      </app-column>
      <app-column field="vacancyLink" header="Контакты" class="contacts">
        <template #body="slotProps">
          <div class="contacts">
            <a
      ...
      <app-column header="Пройденные этапы">
        <template #body="slotProps">
          <span v-if="!slotProps.data.stages">Не указано</span>
          <div v-else class="interview-stages">
            <app-badge v-for="(stage, i) in slotProps.data.stages" :key="i" :value="i + 1" rounded v-tooltip.top="stage.name"></app-badge>
          </div>
        </template>
      </app-column>
      <app-column header="Зарплатная вилка">
        <template #body="slotProps">
          <span v-if="!slotProps.data.salaryFrom">Не указано</span>
